...
public class LottoGenerator {
    ...
    private static final int LOTTO_NUMBER_LOWER_LOMIT = 1;
    ...
    private void validateMoney(int money) {
        if (isZeroOrNegativeNumber(money) || !isDividedByOneThousand(money)) {
            throw new IllegalStateException(MONEY_SHOULD_BE_DIVIDED_BY_ONE_THOUSAND);
        }
    }
    ...
    private Lotto generateLotto() {
        List<Integer> randomNumbers = new ArrayList<>(
                Randoms.pickUniqueNumbersInRange(LOTTO_NUMBER_LOWER_LOMIT, LOTTO_NUMBER_UPPER_LIMIT,
